...
export default Vue.extend({
  ...
  methods: {
    ...
    async deleteCape(cape: Cape) {
      try {
        await this.$axios.delete(`/api/v1/capes/${cape.uuid}`)
    ...
    async addCape(cape: Cape) {
      try {
        await this.$axios.put(`/api/v1/capes`, {
          uuid: cape.uuid,
          type: cape.type
        })
    ...
    async setCapeType(cape: Cape) {
      try {
        await this.$axios.put(`/api/v1/capes/${cape.uuid}`, {
          uuid: cape.uuid,
          type: cape.type
        })
          .then(() =>
  ...
  async fetch() {
   this.capes = (await this.$axios.get('/api/v1/capes?form=true')).data
  }
