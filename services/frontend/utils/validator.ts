export default {
  username: (v: string) => {
    return (!v || (/^(?=.{4,50}$)[A-Za-z0-9]+(?:[-_.][A-Za-z0-9]+)*$/.test(v))) || 'Username should be between 4 and 50 characters and can only contains alphanumeric characters, underscore and dot.';
  },
  password: (v: string) => {
